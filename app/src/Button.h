...
public:
    /**
     * @brief Binds the button to one cached mux state and one source channel.
     *
     ...
     * @retval -EINVAL The mux index or channel number is out of range.
     */
    int init(InputController &inputs, size_t mux_index, uint8_t pin);
