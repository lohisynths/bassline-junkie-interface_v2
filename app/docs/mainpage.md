...
- `MOD`: owns six selector buttons and one knob, stores their configuration tables plus six selector groups with seventeen virtual target banks each, remembers one linked target offset per group, tracks long-press preview state for every selector button, initializes them against shared `InputController` and `LEDSController` objects, applies selector LED updates, and emits the current transition logs
