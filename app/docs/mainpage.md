...
- flash-backed preset persistence through the `EEPROM` backend, the `Preset` controller, and the `PresetSnapshot` model
...
- `LED_DISP`: owns one knob plus one three-digit active-low seven-segment display, stores the preset-selector knob configuration plus the legacy digit segment patterns, initializes itself against shared `InputController` and `LEDSController` objects, renders blank-leading preset numbers on the display, uses a reduced-resolution encoder step for preset browsing, and exposes the display state consumed by `Preset` for save/load gestures and temporary blanking blink feedback
...
- `main.cpp`: initializes the board LED, starts an input thread that constructs `UART`, `MIDI`, `InputController`, `LEDSController`, one `ADSR` block, one `FLT` block, one `LFO` block, one `MOD` block, one `OSC` block, one `EEPROM`, one `Preset`, and one `LED_DISP` block as plain locals, and runs the block update loop alongside the heartbeat LED
...
- A dedicated input thread builds `UART`, `MIDI`, `InputController`, `LEDSController`, the `ADSR`, `FLT`, `LFO`, `MOD`, and `OSC` control blocks, the `EEPROM`, the `Preset` controller, and the `LED_DISP` preset selector, then repeatedly refreshes inputs and updates all blocks.
...
- MOD knob value changes emit MIDI Control Change messages on channel `1`, using CC `0..107` across the 108 MOD modulation values, and MOD bank recalls also resend the currently active virtual-bank value.
...
- Preset loads also resend the full MOD MIDI Control Change snapshot, including all 108 MOD modulation values.
...
- Flash-backed preset persistence is implemented in `src/EEPROM.cpp` and declared in `src/EEPROM.h`.
- High-level preset control is implemented in `src/Preset.cpp` and declared in `src/Preset.h`.
